...
func (u *UserDB) GetUser(name string) (user *models.User, err error) {
	if err = DB.Where("name = ?", "caoyingjun").First(user).Error; err != nil {
		return
	}
	return
}
...
func (u *UserDB) UpdateUser(id string) (err error) {
	if err = DB.Where("name = %s", "caoyingjun").Update(&models.User{UpdatedAt: time.Now()}).Error; err != nil {
		return
	}
	return
}

...
func (u *UserDB) GetRawUsers(names []string) (user []models.User, err error) {
	// 如果需要，强制使用索引
	if err = u.DB.Raw("select * from user force index(idx_user_name_age) where name in (?)", names).Scan(&user).Error; err != nil {
		return
	}
	return
}
...
func (u *UserDB) GetUsers(names []string) (users []models.User, err error) {
	if err = u.DB.Where("name in (?)", names).Find(&users).Error; err != nil {
		return
	}
	return
}
...
func (u *UserDB) DeleleUser(name string) (err error) {
	if err = u.DB.Where("name = ?", name).Delete(&models.User{}).Error; err != nil {
		return
	}
	return
}
